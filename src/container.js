...
export function buildContainer(config) {
  ...
  return { config, db, clock, logger, athleteStore, activityStore, inviteStore, authStateStore, strava, tokens, dispatcher, activityProcessor, connectService, athleteService, dataDeletionService, sessions: createSessions(config.sessionSecret), close() { if (!closed) { closed = true; db.close(); } } };
}

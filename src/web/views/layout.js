...
const siteFooter = (config, blurb) => html`<footer class="home-footer"><span>${raw(blurb ?? (config?.supportEmail ? html`<a href="mailto:${config.supportEmail}">${config.supportEmail}</a>` : ''))}</span><span class="built-by">Built by <a href="https://schmoller.nl" target="_blank" rel="noopener noreferrer" aria-label="Built by Schmoller Agile Development"><img src="/logo-schmoller.svg" alt="" class="built-by-mark"></a></span><div><a href="/privacy">Privacy</a><a href="/support">Support</a></div></footer>`;
/**
 * @param {string} title
 * @param {string} body
...
export const page = (title, body, { mainClass = '', loginUrl = null, loggedIn = false, config = null, footerBlurb = null } = {}) => html`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="#f8f4ed"><title>${title}</title><style>
  ...
  * { box-sizing: border-box; }
  ...
  .activity-card { position: relative; overflow: hidden; border: 1px solid rgba(27,36,48,.1); border-radius: 18px; background: #fffdf9; box-shadow: 18px 22px 0 #ece4d9; transform: rotate(2.5deg); margin-right: 24px; }
  .activity-header { display: flex; align-items: flex-start; gap: 12px; padding: 21px 23px 0; }
  .activity-avatar { flex: none; display: flex; align-items: center; justify-content: center; width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, #ff7800, #f8274c 55%, #a70a9d); color: #fff; font-size: .88rem; font-weight: 800; }
  .activity-who { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 2px; }
  .activity-name { font-size: .94rem; font-weight: 750; letter-spacing: -.02em; color: #1b2430; }
  .activity-meta { color: #8f97a1; font-size: .76rem; line-height: 1.4; }
  .activity-chevron { flex: none; margin-top: 8px; color: #b9c0c8; }
  .activity-body { padding: 13px 23px 22px; }
  .activity-type { display: flex; align-items: center; margin-bottom: 5px; color: #8f97a1; }
  .activity-card h2 { margin: 0 0 12px; font-size: 1.42rem; letter-spacing: -.045em; }
  .activity-stats { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-top: 14px; padding: 16px 0; border-top: 1px solid #eee8df; border-bottom: 1px solid #eee8df; } .activity-stats span { color: #78838d; font-size: .68rem; font-weight: 750; letter-spacing: .06em; text-transform: uppercase; } .activity-stats strong { display: block; margin-top: 4px; color: #252d36; font-size: 1rem; letter-spacing: -.03em; }
  .route { height: 116px; margin-top: 16px; border-radius: 10px; background: linear-gradient(135deg, #d3e2c8, #eef1e9 55%, #d9e7d1); overflow: hidden; } .route svg { width: 100%; height: 100%; } .activity-message { margin: 0; color: #3d454e; font-size: .89rem; line-height: 1.55; } .activity-goal-divider { color: #7a858f; }
  .activity-footer { display: flex; align-items: center; gap: 10px; margin-top: 16px; padding-top: 14px; border-top: 1px solid #eee8df; }
  .activity-kudos-avatars { display: flex; }
  .activity-kudos-avatars span { width: 21px; height: 21px; margin-left: -7px; border-radius: 50%; border: 2px solid #fffdf9; background: linear-gradient(135deg, #e7c9a9, #c98a56); } .activity-kudos-avatars span:first-child { margin-left: 0; } .activity-kudos-avatars span:nth-child(2) { background: linear-gradient(135deg, #b9c8d8, #6f8aa3); } .activity-kudos-avatars span:nth-child(3) { background: linear-gradient(135deg, #cbb8db, #8b6ba3); }
  .kudos-count { flex: 1; color: #68737d; font-size: .82rem; font-weight: 650; }
  .activity-footer-actions { display: flex; gap: 4px; }
  .activity-footer-actions button { display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; border: 0; border-radius: 50%; background: transparent; color: #9099a3; cursor: default; }
  ...
  .invite-request { padding: 0 0 92px; border-top: 1px solid #e0d7cc; margin-top: 75px; } .invite-form { display: grid; gap: 12px; margin-top: 28px; max-width: 420px; padding: 24px; border: 1px solid #dfd5ca; border-radius: 18px; background: #fffdf9; } .invite-form label { font-size: .9rem; font-weight: 720; } .invite-form input[type="text"], .invite-form input[type="email"] { width: 100%; padding: 12px; border: 1px solid #cfc3b6; border-radius: 10px; font: inherit; } .field-honeypot { position: absolute; left: -9999px; width: 1px; height: 1px; overflow: hidden; } .form-success { margin-top: 28px; padding: 14px 18px; border-left: 3px solid #f35b3c; background: #fff1e9; font-weight: 650; }
  .home-footer { display: grid; grid-template-columns: 1fr auto 1fr; align-items: center; gap: 20px; padding: 25px 0 42px; border-top: 1px solid #e0d7cc; color: #68737d; font-size: .86rem; } .home-footer div { display: flex; gap: 18px; justify-content: flex-end; }
  .built-by { display: inline-flex; align-items: center; justify-content: center; gap: 5px; } .built-by a { display: inline-flex; align-items: center; opacity: .82; transition: opacity .15s; } .built-by a:hover { opacity: 1; } .built-by-mark { height: 30px; width: auto; vertical-align: middle; }
  @media (max-width: 720px) { .home-footer { grid-template-columns: 1fr; text-align: center; } .home-footer div { justify-content: center; } }
